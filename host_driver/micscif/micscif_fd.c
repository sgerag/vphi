...
reg_err:
	...
	case SCIF_REGV:
	{
		...
		if ((ret = __scif_registerv(priv->epd, sg_buf, regv.nr_pages << PAGE_SHIFT,
				regv.offset, regv.prot, regv.flags)) < 0) {
			err = (int)ret;
...
//		for (j = 0; j < i; j++) {
		__scif_unregister(priv->epd, ret, (regv.nr_pages) << PAGE_SHIFT);
