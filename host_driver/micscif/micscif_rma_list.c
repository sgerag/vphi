...
//printk(KERN_ERR "%s %d TEST\n", __func__, __LINE__);
		window = list_entry(item, 
			struct reg_range_t, list_member);
//	printk(KERN_ERR "%s %d offset: 0x%lx, end_offset: 0x%lx\n", __func__, __LINE__, offset, end_offset);
...
//	printk(KERN_ERR "%s %d not error0\n", __func__, __LINE__);
					return 0;
	}
				/* Extra logic for full windows */
				if (offset == end_offset){
					/* Spanning multiple whole windows */
//	printk(KERN_ERR "%s %d not error1\n", __func__, __LINE__);
					return 0;
				}
