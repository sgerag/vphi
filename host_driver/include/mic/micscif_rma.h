 ...
 */
static __always_inline int
micscif_detect_large_pagev(int idx, struct scif_pinned_pages *pinned_pages, char *addr)
{
	int nr_pages, huge;
	char *next_huge, *end;
	char *end_addr = addr + (1 << PAGE_SHIFT);

	while (addr < end_addr) {
		huge = micscif_is_huge_page(pinned_pages, idx);
		if (huge) {
			next_huge = (char *)ALIGN(
				(unsigned long)(addr + 1), 
				PMD_SIZE);
			...
			nr_pages = (int)((end - addr) >> PAGE_SHIFT);
			pinned_pages->num_pages[idx] = (int)nr_pages;
			addr = end;
		} else {
			pinned_pages->num_pages[idx] = 1;
			addr += PAGE_SIZE;
			ms_info.nr_4k_pages++;
		}
		pinned_pages->nr_contig_chunks++;
