		...
		int map_flags, scif_pinned_pages_t *pages)
{
	...
	/* addr/len must be page aligned. len should be non zero */
	if ((!len) ||
		(align_low((uint64_t)sg_addrs->addr, PAGE_SIZE) != (uint64_t)sg_addrs->addr) ||
		(align_low((uint64_t)len, PAGE_SIZE) != (uint64_t)len))
		return -EINVAL;
...
//		start_page = pinned_pages->pages;
//		start_vma = pinned_pages->vma;
		for (i = 0; i < nr_pages; i++) {
...
#endif
			for (i = 0; i < nr_pages; i++) {
				micscif_detect_large_pagev(i, pinned_pages, sg_addrs[i].addr);
			}
//			micscif_detect_large_page(pinned_pages, addr);
		} else {
			if (try_upgrade) {
					...
					int prot, int map_flags)
{
	...
	pr_debug("SCIFAPI register: ep %p %s addr %p len 0x%lx"
		" offset 0x%lx prot 0x%x map_flags 0x%x\n", 
		epd, scif_ep_states[epd->state], sg_addrs->addr, len, offset, prot, map_flags);
	...
	atomic_set(&pinned_pages->ref_count, pinned_pages->nr_pages);
	...
	pr_debug("SCIFAPI register: ep %p %s addr %p"
		" len 0x%lx computed_offset 0x%llx\n", 
		epd, scif_ep_states[epd->state], sg_addrs->addr, len, computed_offset);
...
scif_mmap(struct vm_area_struct *vma, scif_epd_t epd)
{
	...
	//vma->vm_flags |= VM_DONTCOPY | VM_DONTEXPAND | VM_DONTDUMP | VM_PFNPHI;
#else
	...
	//vma->vm_flags |= VM_DONTCOPY | VM_DONTEXPAND | VM_RESERVED | VM_PFNPHI;
#endif
