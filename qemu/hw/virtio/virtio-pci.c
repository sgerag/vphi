...
static int virtio_scif_pci_init(VirtIOPCIProxy *vpci_dev)
{
        DEBUG_IN();
    VirtIOScifPCI *dev = VIRTIO_SCIF_PCI(vpci_dev);
    DeviceState *vdev = DEVICE(&dev->vdev);
    DeviceState *proxy = DEVICE(vpci_dev);
    char *bus_name;
...
static Property virtio_scif_pci_properties[] = {
    DEFINE_PROP_BIT("ioeventfd", VirtIOPCIProxy, flags,
                    VIRTIO_PCI_FLAG_USE_IOEVENTFD_BIT, true),
    DEFINE_PROP_UINT32("vectors", VirtIOPCIProxy, nvectors, 3),
    DEFINE_PROP_UINT32("class", VirtIOPCIProxy, class_code, 0),
...
static void virtio_scif_pci_class_init(ObjectClass *klass, void *data)
{
DEBUG_IN();
    DeviceClass *dc = DEVICE_CLASS(klass);
    VirtioPCIClass *k = VIRTIO_PCI_CLASS(klass);
    PCIDeviceClass *pcidev_k = PCI_DEVICE_CLASS(klass);
    k->init = virtio_scif_pci_init;
    set_bit(DEVICE_CATEGORY_INPUT, dc->categories);
    dc->props = virtio_scif_pci_properties;
    pcidev_k->vendor_id = PCI_VENDOR_ID_REDHAT_QUMRANET;
    pcidev_k->device_id = 0x1011;
    pcidev_k->revision = VIRTIO_PCI_ABI_VERSION;
    pcidev_k->class_id = PCI_CLASS_COMMUNICATION_OTHER;
}

static void virtio_scif_pci_instance_init(Object *obj)
{
//    VirtIOScifPCI *dev = VIRTIO_SCIF_PCI(obj);
//        DEBUG_IN();
//    object_initialize(&dev->vdev, sizeof(dev->vdev), "virtio-scif");
//    object_property_add_child(obj, "virtio-backend", OBJECT(&dev->vdev), NULL);
    VirtIOScifPCI *dev = VIRTIO_SCIF_PCI(obj);
...
static const TypeInfo virtio_scif_pci_info = {
    .name          = TYPE_VIRTIO_SCIF_PCI,
    .parent        = TYPE_VIRTIO_PCI,
    .instance_size = sizeof(VirtIOScifPCI),
    .instance_init = virtio_scif_pci_instance_init,
    .class_init    = virtio_scif_pci_class_init,
};
...
static void virtio_pci_register_types(void)
{
	...
	type_register_static(&virtio_scif_pci_info);
}
